...
void TileMap::renderTile(int rowIndex, int columnIndex, Camera * camera)
{
	...
	int xV = 0, yV = 0;
	camera->getWorldToViewLocation(xW, yW, xV, yV);
...
void TileMap::render(Camera* camera)
{
	...
	int colEnd = (camera->getRight() + tileWidth - 1) / tileWidth - 1;

	int rowBegin = (worldHeight - camera->getTop()) / tileHeight ;
	int rowEnd = ((worldHeight - camera->getBottom()) + tileHeight - 1) / tileHeight - 1;
